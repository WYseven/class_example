...
(function(){
    ...
    function renderNav(pathNav,id){
        ...
        for( var i = 0; i < parents.length-1; i++ ){
            str += '<a href="javascript:;" style="z-index:'+(n--)+';" data-file-id="'+parents[i].id+'">'+parents[i].name+'</a>';
        }

        str += '<a class="current-path" style="z-index:'+(n--)+';" href="javascript:;" data-file-id="'+parents[parents.length-1].id+'">'+parents[parents.length-1].name+'</a>' 
    ...
    function movefileHandle(){
        ...
        newDiv.appendChild(treeHtml(-1));
         ...
         //取消冒泡
         tools.$(".full-pop")[0].onmousedown  =function (ev){
              ev.stopPropagation();   
         }
         tools.$(".mask")[0].onmousedown  =function (ev){
              ev.stopPropagation();   
         }
         var firstName = tools.$(".file-title",select[0])[0];
         tools.$(".file-name",tools.$(".full-pop")[0])[0].innerHTML = firstName.innerHTML;
         tools.$(".total",tools.$(".full-pop")[0])[0].innerHTML = "等"+select.length+"个文件";
         //找到弹框中div
         var treeDiv = tools.$("div",treeMenu);
    ...
    function dialogTreeEvents(){
        ...
        tools.addEvent(dialogTreeMenu,"click",function(ev){
            ...
            tools.$(".confirm",tools.$(".full-pop")[0])[0].style.opacity = 1;
            for( var i = 0; i < arr.length; i++ ){
                if( fileId == arr[i].id ){
                    error.innerHTML = "不能将文件移动到自身或其子文件夹下";
                    tools.$(".confirm",tools.$(".full-pop")[0])[0].style.opacity = ".5";
                    break;
                }
            }
            //提示当前文件夹下已有
    ...
    tools.addEvent(create,"mouseup",function(){
        ...
        edtor.select();
        edtor.onkeydown = function (ev){
            if( ev.keyCode === 13 && edtor.value.trim()){
               handleCreateFile(); 
            }
        };
    });
    ...
    function handleCreateFile(){
        if(newStructrue && edtor && !edtor.value.trim()) {
            ...
            if( isNameExsit(data.files,pid,edtor.value.trim()) ){
                tools.removeChild(fileList,newStructrue);
    ...
    function renameHandle(){
        ...
        if( select.length > 1 ){  //说明没有选中
            ...
            edtorInput.onkeydown = function (ev){
                if( ev.keyCode === 13 && edtorInput.value.trim()){
                   renameDownHandle(); 
                }
            };
            edtorInput.onmousedown = function (ev){
                ev.stopPropagation();
            };
            edtorInput.onclick = function (ev){
                ev.stopPropagation();
            };
        }
    }
    //在document上dowm，配合rename
    tools.addEvent(document,'mousedown',renameDownHandle);
    function renameDownHandle(){
        //如果有修改的内容
        if(titleIng && fileEdtor){
            tools.hide(fileEdtor);
            tools.show(titleIng);
            //如果名字已经存在，要提醒
            if(isNameExsit(data.files,pid,edtorInput.value,rename.fileId) ){
