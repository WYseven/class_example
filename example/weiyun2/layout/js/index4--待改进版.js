...
(function(){
    ...
    function postionLevel(id){
        ...
        console.log( document.querySelector(".tree_title[data-file-id='"+prev+"']") );
        tools.addClass(document.querySelector(".tree_title[data-file-id='"+prev+"']"),"tree-nav");
        
       for( var i = 0; i < parents.length; i++ ){
           var oDiv = document.querySelector(".tree_title[data-file-id='"+parents[i].id+"']");
           if( oDiv.nextElementSibling ){
                oDiv.nextElementSibling.style.display = "block";
           }
       }
       //tools.addClass(oDiv,"tree-nav");
      /* prev = oDiv.dataset.fileId;
       console.log( oDiv );*/
    }
    ...
    function positionTree(fileId){
        var diva = document.querySelector(".tree_title[data-file-id='"+fileId+"']");
        var prevDiv = document.querySelector(".tree_title[data-file-id='"+prev+"']");

        tools.removeClass(prevDiv,"tree-nav");

        tools.addClass(diva,"tree-contro");
        tools.addClass(diva,"tree-nav");

        if(diva.nextElementSibling){
            diva.nextElementSibling.style.display = "block";
        }
        //让当前之上所有的ul显示出来
        postionLevel(fileId);
