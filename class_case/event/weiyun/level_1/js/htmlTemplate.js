...
function positionTreeById( positionId ){
	...
	var ele = getElementByDataId("tree-title",positionId);
...
function getElementByDataId(classNams,id){
	var allElement = tools.$("."+classNams);

	for( var i = 0; i < allElement.length; i++ ){
		if( allElement[i].dataset.fileId == id ){
			return allElement[i];
		}
	}
}