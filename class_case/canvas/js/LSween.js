function MTween(obj,attrObj,duration,fx,callBack){
	var newObj = {};  //声明一个空对象，用于储存数据
	for(var attr in attrObj){  //for in 循环传进来的参数
		newObj[attr] = {};	//给新的对象赋以key值
	}
	for(var attr in attrObj){  //for in循环传进来的参数
		//给key值赋以对象
		//在里面得到初始值，知道定义的值，得到运动的实际距离； 每个都对应定义的距离，初始的距离，得到运动的距离
	...
	obj.timer = setInterval(function (){
		...
		if( t >= d ){
			clearInterval(obj.timer);
			obj.timer = null;//停止了定时器后把元素的定时器清空
